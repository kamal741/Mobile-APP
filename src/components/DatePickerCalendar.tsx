...
/**
 * Pure calendar content — no <Modal> wrapper.
 * Render this directly inside whatever single Modal owns the screen
 * (e.g. DateTimePickerModal's outer Modal) to avoid stacking nested
 * Modals, which on Android causes touch coordinates to be computed
 * against the wrong window and results in the wrong day being selected.
 *
 * DatePickerModal wraps this in its own <Modal> for standalone use.
 */
export function DatePickerCalendar({ value, onConfirm, onDismiss, minDate }: Props) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  // Always normalise min to midnight so the < comparison is purely calendar-day.
