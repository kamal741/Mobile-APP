...
import { Calendar, Home, Star, Camera } from 'lucide-react-native';
import { agentClientHistoryUrl } from '../../../../lib/tourApi';
import { useAgentOffers } from '../../../../lib/agentApi';
...
export function TourHistoryScreen() {
  ...
  const { data: clientHistory, isLoading } = useQuery<any>({
    queryKey: [agentClientHistoryUrl(clientId)],
    enabled: !!clientId,
  });

  // Fetch offer count for this client — size:1 is enough since we only need totalElements
  const { data: offersData } = useAgentOffers(
    { clientProfileId: clientId, size: 1 },
    { enabled: !!clientId },
  );
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      {clientHistory?.summary && (
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <Text style={styles.statValue}>{offersData?.totalElements ?? 0}</Text>
            ...
            <CardHeader>
              <View style={styles.tourHeaderRow}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: getStatusColor(tour.status) + '20' }]}>
                  <Text style={[styles.statusText, { color: getStatusColor(tour.status) }]}>{formatStatus(tour.status)}</Text>
                </View>
              </View>
            </CardHeader>
            <CardContent>
                      ...
                      {property.rating && (
                        <View style={styles.ratingRow}>
                          {[1, 2, 3, 4, 5].map((starNumber) => (
                            <Star
                              key={`${property.id}-${starNumber}`}
                              size={14}
                              color={starNumber <= property.rating.rating ? '#facc15' : '#d1d5db'}
                              fill={starNumber <= property.rating.rating ? '#facc15' : 'transparent'}
                            />
                          ))}
                        </View>
                      )}
