...
export function ClientMediaScreen() {
  ...
  const { data: clientMedia, isLoading } = useQuery<any[]>({
    queryKey: [`/api/client/v1/client/properties/${propertyId}/media`],
    enabled: !!propertyId,
  });
  ...
  const photos =
    clientMedia?.filter((m: any) => m.mediaType === 'IMAGE' || m.contentType?.startsWith('image/')) || [];
  const videos =
    clientMedia?.filter((m: any) => m.mediaType === 'VIDEO' || m.contentType?.startsWith('video/')) || [];
