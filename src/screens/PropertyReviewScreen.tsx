...
import { catalogPropertyReviewsPath } from '../lib/apiGlobalPaths';

/** Set true when the backend exposes per-tour property ratings / agent review routes (legacy `/api/tours/...` not on Spring). */
const TOUR_PROPERTY_REVIEW_API = false;
...
export function PropertyReviewScreen() {
  ...
  const { data: existingRating } = useQuery<any>({
    ...
    enabled: !isAgent && TOUR_PROPERTY_REVIEW_API,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: [catalogPropertyReviewsPath(propertyId)] });
  ...
  if (tourId && !TOUR_PROPERTY_REVIEW_API) {
    return (
      <View style={styles.unavailableWrap}>
        <Text style={styles.unavailableTitle}>Tour reviews unavailable</Text>
        <Text style={styles.unavailableBody}>
          Per-tour property ratings and agent review endpoints are not implemented on the current API. You can open the
          property from the tour to browse listing details and catalog reviews.
        </Text>
